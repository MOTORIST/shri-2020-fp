...
export const validateFieldN3 = (shapes) => {
    ...
    const isEqualRedShapesAndBlueShapes = pipe(values, countBy(identity), isRedEqualsBlue);
...
export const validateFieldN5 = (shapes) => {
    ...
    return pipe(values, countBy(identity), omit([COLORS.WHITE]), values, any(gteThree))(shapes);
}

// 6. Две зеленые фигуры (одна из них треугольник), еще одна любая красная.
export const validateFieldN6 = (shapes) => {
    const colors = invert(shapes);
    const hasTriangleGreenShapes = colors => indexOf(SHAPES.TRIANGLE, colors[COLORS.GREEN]) === 1;
    const hasGreenShapes = has(COLORS.GREEN);
    const hasRedShapes = has(COLORS.RED);
    
    return allPass([hasGreenShapes, hasTriangleGreenShapes, hasRedShapes])(colors);
}
...
export const validateFieldN7 = (shapes) => {
    ...
    return pipe(values, all(isOrange))(shapes);
}
...
export const validateFieldN9 = (shapes) => {
    const isOrange = equals(COLORS.GREEN);
